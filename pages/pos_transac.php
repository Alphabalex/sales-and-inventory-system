                ...
                for($i=1; $i<=$countID; $i++)
                  {
                    ...
                    $query = "INSERT INTO `transaction_details`
                               (`ID`, `TRANS_D_ID`, `PRODUCTS`,`CODE`, `QTY`, `PRICE`, `EMPLOYEE`, `ROLE`)
                               VALUES (Null, '{$today}', '".$_POST['name'][$i-1]."', '".$_POST['code'][$i-1]."', '".$_POST['quantity'][$i-1]."', '".$_POST['price'][$i-1]."', '{$emp}', '{$rol}')";
                    ...
                    if ($cash<$total) {
                      ...
                      (CUST_ID,CREDIT_AMOUNT,TRANS_ID,`DATE`, STATUS)
                      VALUES('{$customer}','{$bal}','{$today}','{$date}','Unpaid')";
